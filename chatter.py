...
gptmodel = GPT4All("ggml-model-gpt4all-falcon-q4_0.bin")
    ...
    while True:
        try:
            with gptmodel.chat_session():
                ...
                # response = gptmodel.generate(str(messages))
                response = gptmodel.generate(str(messages[-1]['content']))
            ...
            print(f'\n\nError communicating with Gpt4: "{oops}"')
...
def get_response(message,content,enable_kb=False):
    con = sqlite3.connect(os.path.join(script_dir,'chatter.db'))
    cursor = con.cursor()
...
update_db()
bot.infinity_polling(timeout=550,long_polling_timeout=550)
